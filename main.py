...
class Main(tk.Frame):
    ...
    def init_main(self):
        ...
        self.tree = ttk.Treeview(self, columns=('ID', 'sportsman', 'league', 'team'), height=15,
                                 show='headings')

        self.tree.column('ID', width=30, anchor=tk.CENTER)
        self.tree.column('sportsman', width=290, anchor=tk.CENTER)
    ...
    # Добавление данных
    def records(self, sportsman, league, team):
        self.db.insert_data(sportsman, league, team)
        self.view_records()

    # Обновление данных
    def update_record(self, sportsman, league, team, ID):
        self.db.c.execute('''UPDATE athletes SET sportsman=?, league=?, team=?, WHERE ID=?''',
                          (sportsman, league, team, ID,  self.tree.set(self.tree.selection()[0], '#1')))
        self.db.conn.commit()
        self.view_records()
...
class Child(tk.Toplevel):
    ...
    def init_child(self):
        ...
        label_select = tk.Label(self, text='Лига')
        ...
        self.combobox = ttk.Combobox(self, values=[u'РПЛ', u'ФНЛ', u'NXL', 'КХЛ'])
